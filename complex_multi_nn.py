...
def create_meta_model(input_dim, num_classes):
    model = tf.keras.Sequential([
        tf.keras.layers.InputLayer(input_shape=(input_dim,)),
...
meta_model = create_meta_model(input_dim=train_feature_preds.shape[1], num_classes=len(np.unique(y_train)))
print("Training enhanced meta-model...")
meta_model.fit(train_feature_preds, y_train, epochs=20, batch_size=64, verbose=0)  # Increased epochs
