...
# Ensure 'Storm_Intensity' is categorical
...
# Step 3: Train separate models for each feature
feature_models = []  # List to store models for each feature
for feature in X_train.columns:
    model = RandomForestClassifier(random_state=42)  # Random Forest for each feature
...
# Step 5: Train a meta-model on feature-specific predictions
...
# Feature Importance Visualization for Each Feature Model
for i, model in enumerate(feature_models):
    ...
    plt.yticks(range(len(model.feature_importances_)), [X.columns[i]])
    plt.xlabel("Importance")
    plt.title(f"Feature Importance for Model {i+1} ({X.columns[i]})")
...
plt.savefig('plots/actual_vs_predicted.png')  # Save scatter plot of actual vs predicted
plt.close()